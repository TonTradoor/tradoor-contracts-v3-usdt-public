...
import { JettonBurn, JettonTransfer, MockJettonWallet } from "../build/JettonMock/tact_MockJettonWallet";


export * from '../build/JettonMock/tact_MockJettonWallet';

export class ExtendJettonWallet extends MockJettonWallet {
    ...
    static async fromInit(owner: Address, jetton_master: Address) {
        const base = await MockJettonWallet.fromInit(owner, jetton_master)
        if (base.init === undefined) {
            throw new Error("JettonWallet init is not defined")
        }
        return new ExtendJettonWallet(base.address, {code: base.init.code, data: base.init.data})
    }
