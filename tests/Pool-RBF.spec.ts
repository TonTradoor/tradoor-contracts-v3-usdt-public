...
describe('Pool', () => {
    ...
    beforeEach(async () => {
        ...
        compensator = await blockchain.treasury('compensator');
        user0 = await blockchain.treasury('user0');
        user1 = await blockchain.treasury('user1');

        // deploy orderBook
        const orderBookDeployResult = await orderBook.send(
            deployer.getSender(),
            {
                value: toNano('0.5'),
            },
        ...
        // deploy pool
        const poolDeployResult = await pool.send(
            deployer.getSender(),
            {
                value: toNano('0.5'),
            },
        ...
        let orderBookWalletAddress = await jetton.getGetWalletAddress(orderBook.address);
        const setOderBookConfigResult = await orderBook.send(
            deployer.getSender(),
            {
                value: toNano('0.1'),
            },
            {
                ...
                enableExecutor: true,
                compensator: compensator.address,
                ...
                usdtWallet: orderBookWalletAddress,
                pool: pool.address
            }
        );
        ...
        expect(orderBookConfigData.usdtWallet).toEqualAddress(orderBookWalletAddress);
        expect(orderBookConfigData.isExecutor).toBeTruthy();
        expect(orderBookConfigData.isCompensator).toBeTruthy();


        // set config to pool
        const setPoolConfigResult = await pool.send(
            deployer.getSender(),
            {
                value: toNano('0.1'),
            },
