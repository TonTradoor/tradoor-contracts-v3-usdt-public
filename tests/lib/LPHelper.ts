...
export async function createIncreaseLiquidityOrder(user: SandboxContract<TreasuryContract>,
    params: { liquidity?: number, executionFee?: number, gas?: number, forwardTon?: number }) {

    const liquidity = params.liquidity || 10000;
    const executionFee = params.executionFee || 0.1;
    const gas = params.gas || 0.2;
    const forwardTon = params.forwardTon || 0.1;
    ...
    const trxResult = await jettonWallet.send(
        ...
        },
        {
            ...
            custom_payload: null,
            forward_ton_amount: toNano(executionFee + forwardTon),
            forward_payload:
                beginCell()
                    .storeUint(1, 1)
                    .storeRef(
